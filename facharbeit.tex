...
Der Client übersendet dieses Identitätspacket an den Server, welcher dies durch den gespeicherten öffentlichen Schlüssel (\textit{rec\_pub\_key}) des (aus seiner Sicht) Empfängers überprüft. Sollte die Signatur nicht valide sein, wird die Verbindung abgebrochen. Folgender Quellcode wird verwendet um die Identität zu überprüfen:
\begin{minted}[breaklines]{rust}
...
Wir gehen nun davon aus, dass der Client an den Server eine Nachricht versenden möchte. Er ruft nun zunächst den öffentlichen Schlüssel des Empfänges aus dem Datenspeicher ab und verschlüsselt die Nachricht. Anschließend übersendet er die verschlüsselte Nachricht an den Server.
\begin{minted}[breaklines]{rust}
