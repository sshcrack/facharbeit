...
\begin{comment}
Ein dezentrales Netzwerk besteht jedoch aus mehreren Servern bzw. Clients, wodurch die Angriffsfläche vergrößert wird und es sich als schwieriger erweist, das gesamte Netzwerk zu kompromittieren bzw. außer Gefecht zu setzen \vcite{GeeksCenralizeDecentralized}.
Da Clients und Server des dezentralen Netzwerkes sich gegenseitig nicht "vertrauen", können selbst durch eine Kompromittierung nicht alle Daten des Netzwerkes erfasst werden oder der gesamte Datenverkehr des Netzwerkes überwacht werden \vcite{MessagingNetwork}.
%//REVIEW - Hier gibt es irgendwie keine Quellen, maybe einfach logisch?
Im Anwendungsbeispiel des Messengers ist es wichtig, dass nur Gesprächsteilnehmer (also Sender / Empfänger) Teil am Netzwerk sind, sodass ein Angreifer durch simples beitreten des Netzwerkes nicht die Onion-Adressen der Sender und Empfänger auslesen kann.
\end{comment}
...
Um den Tor-Client zu starten, muss zuerst eine $torrc$ Config-Datei geschrieben werden:
\begin{minted}{rust}
...
\end{minted}
Der Tor-Proxy wurde nun gestartet und wartet auf Anfragen auf einem freien Ports des Computers. Zudem wird ein Onion Service in dem Tor-Netzwerk etabliert, sodass andere Messenger sich mit diesem Service verbinden können.
