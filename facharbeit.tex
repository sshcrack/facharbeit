...
Der zweite zentrale Aspekt dieser Arbeit ist die Anonymität über das Internet. Bei dem normalen Routing, wie wir es tagtäglich nutzen, kommuniziert der Client direkt mit dem Zielserver. Der Zielserver kann hierbei die IP-Adresse des Clients sehen, somit ist eine Rückverfolgung möglich \vcite{LocPolice,TCP_IP}. Und genau bei diesem Problem setzt das Tor-Netzwerk an. Das Tor-Netzwerk besteht aus vielen Servern, genannt \nodes \vcite{TorStructure}. Der Client verschlüsselt die eigentliche Anfrage mehrmals, hüllt sie in ein mehrere "Schalen" ein, welche eine \onion bilden, und leitet diese über drei \nodes an den Zielserver \vebd. Anfangs wird diese \onion zu der \entryn geschickt \vebd. Bei jeder \node, also auch der \entryn, wird eine Schale der \onion "geschält" (die Nachricht einmal entschlüsselt), welche den nächsten Knotenpunkt enthält, zu welchem die Anfrage zunächst geschickt wird \vcite{TorStructure2}. Nur die \entryn weiß somit die reale IP-Adresse des Clients und \textbf{nur} die letzte Node (\exitn) weiß, an welchen Zielserver die Anfrage geschickt wurde \vebd. Wichtig hierbei ist allerdings, dass die \entryn nicht weiß, was der Zielserver der Anfrage ist \vebd. Die \exitn weiß im Gegenzug die reale IP-Adresse des Clients nicht.
