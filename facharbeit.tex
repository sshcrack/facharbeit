...
Der zweite zentrale Aspekt dieser Arbeit ist die Anonymität über das Internet. Bei dem normalen Routing, wie wir es tagtäglich nutzen, kommuniziert der Client direkt mit dem Zielserver. Der Zielserver kann hierbei die IP-Adresse des Clients sehen, somit ist eine Rückverfolgung möglich \vcite{LocPolice,TCP_IP}. Und genau bei diesem Problem setzt das Tor-Netzwerk an. Das Tor-Netzwerk besteht aus vielen Servern, genannt \nodes \vcite{TorStructure}. Der Client verschlüsselt die eigentliche Anfrage mehrmals, nun genannt \onion, und leitet diese über drei \nodes an den Zielserver \vebd. Bei jeder \node wird eine Schale der \onion "geschält", welche den nächsten Knotenpunkt enthält \vcite{TorStructure2}. Die \node, mit welcher der Client zuerst die Verbindung aufbaut, wird \entryn genannt. Nur sie weiß die reale IP-Adresse des Clients. Nun wird diese Anfrage, wie bereits beschrieben über \relayns geleitet \vebd. Die letzte \node, welche die letzte Schale der \onion "schält", wird \exitn genannt \vcite{TorStructure}. Diese \node baut nun eine Verbindung zum Zielserver auf, ohne dass dieser die IP-Adresse des Clients kennt \vebd.

\begin{figure}[h]
  \centering
  \includegraphics[height=\textheight]{tor_vertical.png}
